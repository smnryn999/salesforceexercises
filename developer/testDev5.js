...
const test = ["Which of the following statements cannot be said about Aura components?", "In a company called Página Blanca Sales Inc., Rose, Salesforce Developer, need to update 500 thousands opportunity records. Which code snippet should she use?", "Which of the following is true?", "In a Lightning Web Component (LWC) parent-child relationship, what is the purpose of the @api decorator in the child component?",
             ...
             "When integrating with an external REST API in Salesforce, which HTTP method is typically used for making a request to retrieve data from the external system?",
             "<code>CustomNotificationType notificationType = [SELECT DeveloperName FROM CustomNotificationType <br> &nbsp;&nbsp;&nbsp;&nbsp; WHERE DeveloperName='Custom_Notification']; <br>Messaging.CustomNotification notification = new Messaging.CustomNotification(); <br> notification.setTitle('Apex Custom Notification'); <br> notification.setBody('The notifications are coming from INSIDE the Apex!'); <br> notification.setNotificationTypeId(notificationType.Id); <br> notification.setTargetId(targetId); <br> notification.send(recipientsIds);</code> <br><br> What is the above code snippet for?",
             "When integrating with an external SOAP-based web service in Apex, which approach is commonly used to generate Apex classes to represent the web service's data structures?", "In a Lightning Web Component (LWC), which lifecycle hook is typically used for performing one-time initialization tasks when the component is created?",
             "<code>public class MyClass { <br> <br> &nbsp; public integer num = 25; <br> <br> &nbsp; public integer method(){ <br> &nbsp; &nbsp; integer num = 15; <br> &nbsp; &nbsp; num = 35; <br> &nbsp; &nbsp; return 5 + this.num; <br> &nbsp; } <br>} <br> <br> MyClass mc = new MyClass(); <br> System.debug(mc.method());</code> <br> What is the output of the code snippet above?",
             "What is Apex not used for?","How can you send a message from one Lightning Web Component (LWC) to another using the Lightning Message Service (LMS)?", "In Salesforce Apex, what HTTP status code is typically returned in an HttpResponse to indicate a successful response when using REST or HTTP callouts?",
             "You are building an Apex class to integrate with an external REST API. Which of the following code snippets correctly demonstrates how to make a callout to the API for creating a data with name and job properties and handle the response?"]; 
const options1 = [["Aura components use a component-based architecture.","They can be used in both Lightning Experience and Salesforce Classic.","Aura components are built using JavaScript on the client-side.","Aura components are deprecated and no longer supported in Salesforce."],
                  ...
                  ["It allows deleting a record in the database from a third-party system.","It queries a record based on an id value in the URI.","We can retrieve some account records from an external system via it.","We can perform operations on the Account class from an external system through it."],
                  ...
                  ["For creating a custom notification type.","For sending an information email.","For informing about Apex custom notification.","For sending a custom notification."],
                  ["Manually write the Apex classes to match the SOAP XML structure.","Use the Schema Builder in Salesforce to generate the Apex classes.",'Utilize the "Generate from WSDL" option to create Apex classes from the web service\'s WSDL file.','Use the "External Services" feature in Salesforce to automatically create the Apex classes.'], ["renderedCallback()","connectedCallback()","errorCallback()","disconnectedCallback()"], [20,30,40,"Error"],
                  ["Managing and organizing Salesforce records in the database.","Writing server-side logic and business processes.","Connecting Salesforce with external services.","Creating visually appealing user interfaces for web applications."],
                  ["Directly invoke the methods and properties of the target LWC from the source LWC.","Include a hidden input field in the HTML and update its value to send messages.","Use the window.postMessage() function to communicate between LWCs.","Import the LMS module and use the LMS.publish() method to send a message."], ["200 OK","400 Bad Request","404 Not Found","500 Internal Server Error"],
                  ["<code>Http http = new Http(); <br> HttpRequest request = new HttpRequest(); <br> request.setEndpoint('https://api.example.com/data'); <br> request.setMethod('POST'); <br> request.setHeader('Content-Type', 'application/json;charset=UTF-8'); <br> request.name = 'Clark Mars'; <br>request.job = 'Salesforce Developer'; <br> HttpResponse response = http.send(request); <br> String responseBody = response.getBody();</code>","<code>Http http = new Http(); <br> HttpRequest request = new HttpRequest(); <br> request.setEndpoint('https://api.example.com/data?name=\"Clark Mars\"&job=\"Salesforce Developer\"'); <br> request.setMethod('PUT'); <br> HttpResponse response = http.send(request); <br> String responseBody = response.getBody();</code>",
                   "<code>Http http = new Http(); <br> HttpRequest request = new HttpRequest(); <br> request.setEndpoint('https://api.example.com/data'); <br> request.setMethod('POST'); <br> request.setHeader('Content-Type', 'application/json;charset=UTF-8'); <br> request.setBody('{\"name\":\"Clark Mars\", \"job\":\"Salesforce Developer\"}'); <br> HttpResponse response = http.send(request); <br> String responseBody = response.getBody();</code>","<code>Http http = new Http(); <br> HttpRequest request = new HttpRequest(); <br> request.setEndpoint('https://api.example.com/data'); <br> request.setMethod('PUT'); <br> request.setBody('{\"name\":\"Clark Mars\", \"job\":\"Salesforce Developer\"}'); <br> HttpResponse response = http.send(request); <br> String responseBody = response.getBody();</code>"]];
const answers = [D,A,B,C,C,D,B,A,C,A,B,A,D,C,B,B,D,D,A,C];

...
const test = ["In Apex, what is the purpose of a constructor method in a class?",
              ...
              "Which HTTP Status Code is returned after the POST request is successful?", "Which of the following query returns the oppurtunities whose close date is within the next three weeks?", "Which communication way in Lightning Web Components (LWC) allows a child component to dispatch an event to its parent component?", "Which Visualforce tag cannot create values that is defined by iterating over a set of data?", "How can we switch to master branch in Git local repository?",
              "<code>HttpRequest request = new HttpRequest(); <br> request.setEndpoint('https://api.example.com/data'); <br> request.setMethod('POST'); <br> request.setHeader('Content-Type', 'application/json'); <br> request.setBody('{\"key\":\"value\"}'); <br> HttpResponse response = new Http().send(request);</code> <br><br> Which of the following is false about the code snippet above?",
              "In the bustling city of Blanca, there is a talented developer named Alex. Alex has a reputation for creating lightning-fast Salesforce applications. One day, the mayor of Blanca approaches Alex with a special project. They need a dynamic, modern and responsive web component to manage citizen requests efficiently. <br><br> What technology should Alex choose to create the dynamic and responsive web component for Blanca's citizen request management system?"];
...
const options1 = [["To define the layout of user interface elements.","To create an instance of an object from a class.","To establish a connection to external APIs.","To handle exceptions and errors in the code."],
                 ...
                 ["@track decorator","@api decorator","Custom Events","Lightning Data Service (LDS)"], ["&lt;apex:form&gt;","&lt;apex:dataList&gt;","&lt;apex:repeat&gt;","&lt;apex:pageBlockTable&gt;"], ["git master","git branch master","git checkout master","git switch master"],
...
const answers = [B,A,A,A,D,C,C,B,A,C,D,A,B,B,D,C,A,C,D,B];
...
pass.onclick = function () {
  ...
  for(var i=0; i<codes.length; i++){
     ...
     var datatypes = ["String", " Date ", "Integer", "Boolean", "Decimal", "override", "virtual", "static","void","class", "LightningElement", "lwc", "Text", "trigger", "HttpRequest", "HttpResponse", "Http"];

...
const test = ['Which of the following Visualforce code snippets correctly creates a command button that calls the "updateRecord" method in the controller when clicked?', 'What does the "trigger.new" keyword refer to in Salesforce Apex triggers?',
             ...
             "Which of the following is the correct syntax for creating a new Apex class?", "Which of the following is the correct way to create a new Map in Apex?", "This Apex method takes the current record's Id as a parameter and returns the related contacts. Which code block has not an error? <br> <code>public class ContactController { <br> public static List<Contact> getContacts(Id accId){ <br> return [SELECT Name  FROM Contact WHERE AccountId = :accId]; <br> }<br>}</code>"];
...
const options1 = [['<code>&lt;apex:commandButton action="{!updateRecord}" value="Update"/&gt;</code>','<code>&lt;apex:commandButton method="updateRecord" value="Update"/&gt;</code>','<code>&lt;apex:commandButton onclick="{!updateRecord}" value="Update"/&gt;</code>','<code>&lt;apex:commandButton actionName="updateRecord" value="Update"/&gt;</code>'],
                 ...
                 ['<code>import { LightningElement, wire } from "lwc"; <br> import listContact from "@salesforce/apex/ContactController.listContact"; <br><br> export default class ApexWireMethod extends LightningElement {<br> <samp>@api recordId;</samp> <br><br> <samp>@wire(listContact, { recordId: "$recordId"}) contacts;</samp> <br> }</code>',"","",
                  '<code>import { LightningElement, api, wire } from "lwc"; <br> import listContact from "@salesforce/apex/ContactController.listContact"; <br><br> export default class ApexWireMethod extends LightningElement {<br> <samp>@api recordId;</samp> <br><br> <samp>@wire(listContact, { recordId: "$recordId"}) contacts;</samp> <br> }</code>']];

...
const test = ['Which of the following Visualforce code snippets correctly creates a command button that calls the "updateRecord" method in the controller when clicked?', 'What does the "trigger.new" keyword refer to in Salesforce Apex triggers?',
             ...
             "<code>public class ContactController { <br><samp>@Auraenabled(cachable=true)</samp> <br> <samp> public static List<Contact> getContacts(Id accId){</samp> <br> <samp><samp>return [SELECT Name  FROM Contact WHERE AccountId = :accId];</samp></samp> <br> }<br>}</code> <br> getContacts() method has a parameter and returns the related contacts. We need to pass the current record's Id to the parameter. Which of the following code provides this?"];
...
const options1 = [['<code>&lt;apex:commandButton action="{!updateRecord}" value="Update"/&gt;</code>','<code>&lt;apex:commandButton method="updateRecord" value="Update"/&gt;</code>','<code>&lt;apex:commandButton onclick="{!updateRecord}" value="Update"/&gt;</code>','<code>&lt;apex:commandButton actionName="updateRecord" value="Update"/&gt;</code>'],
                 ...
                 ['<code>import { LightningElement, wire } from "lwc"; <br> import listContact from "@salesforce/apex/ContactController.listContact"; <br><br> export default class ApexWireMethod extends LightningElement {<br> <samp>@api recordId;</samp> <br><br> <samp>@wire(listContact, { accId: "$recordId"}) contacts;</samp> <br> }</code>',"","",
                  '<code>import { LightningElement, api, wire } from "lwc"; <br> import listContact from "@salesforce/apex/ContactController.listContact"; <br><br> export default class ApexWireMethod extends LightningElement {<br> <samp>@api recordId;</samp> <br><br> <samp>@wire(listContact, { accId: "$recordId"}) contacts;</samp> <br> }</code>']];

...
const test = ["What is the purpose of a Scheduled Flow in Salesforce?", 'In Salesforce Reports, what does the "Group Rows" feature help you achieve?', "In Salesforce, what happens to the detail records when the master record in a master-detail relationship is deleted?",
              "Automatically updates a field on a master record with aggregated values from related detail records <br> What refers to the sentence above?", "Which summarize feature should we use if we want to display the value in the middle of all row values ​​in a column containing numerical values ​​in reports?"];
